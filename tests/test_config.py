...
class TestConfig(unittest.TestCase):
    ...
    def test__validate_tags(self, mock_logger, mock_get):
        ...
        tag_setup = (('nofailover', 'failover_priority', self.config._validate_failover_tags),
                     ('nosync', 'sync_priority', self.config._validate_sync_tags))

        for tag, priority_tag, tag_func in tag_setup:
            # Providing one tag is fine
            for single_param in ({tag: True}, {priority_tag: 1}, {priority_tag: 0}):
                mock_get.side_effect = [single_param] * 2
                self.assertIsNone(tag_func())
            ...
            for inconsistent_state in (
                ...
                mock_logger.warning.assert_called_once_with(
                    'Conflicting configuration between ' + tag + ': %s and ' + priority_tag + ': %s.'
                    + ' Defaulting to ' + tag + ': %s',
