...
def validate_host_port_list(value: List[str]) -> bool:
    ...
    :raises: :class:`~patroni.exceptions.PatroniAssertionError` if validation failed.
    """

    if all([validate_host_port(v) for v in value]):
        return True
    raise PatroniAssertionError("didn't pass the validation")
