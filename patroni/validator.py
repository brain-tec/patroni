...
class IntValidator(object):
    ...
    :ivar aligned: require value to be aligned, conflicts with *base_unit*.
    ...
    >>> IntValidator(base_unit='s', aligned=2)
    Traceback (most recent call last):
        ...
    Exception: Cannot use *aligned* and *base_unit* together
    """
    ...
    def __init__(self, *, min: OptionalType[int] = None, max: OptionalType[int] = None,
        ...
        :param aligned: require value to be aligned, conflicts with *base_unit*.
        ...
        self.min = min
        self.max = max
        if aligned and base_unit:
            raise Exception('Cannot use *aligned* and *base_unit* together')
...
schema = Schema({
    ...
    Optional("thread_stack_size"): IntValidator(min=65536, aligned=65535, expected_type=int, raise_assert=True),
    Optional("log"): {
