...
class Ha(object):
    ...
    def _handle_synchronous_strict_mode(self, dcs_state: SyncState, replication_state: Any) -> bool:
        ...
        if len(replication_state.active) < global_config.min_synchronous_nodes:
            ...
            if (replication_state.sync or replication_state.numsync) and \
                    not replication_state.active and not dcs_state.voters:
                # Remove synchronous_standby_names name from postgresql.conf if there is something,
                # but there are no active nodes which could be added to synchronous_standby_names later.
