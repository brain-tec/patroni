...
\
    && if [ $(dpkg --print-architecture) = 'arm64' ]; then \
        ...
        && curl -sL https://go.dev/dl/go1.19.1.linux-arm64.tar.gz | tar xz -C /usr/local go \
        && export GOPATH=/usr/local/lib/go && export GOROOT=/usr/local/go && export PATH=$PATH:$GOROOT/bin:$GOPATH/bin \
        && mkdir ${GOPATH} \
        && git clone --recurse-submodules https://github.com/kelseyhightower/confd.git $GOPATH/src/github.com/kelseyhightower/confd \
        && make -C $GOPATH/src/github.com/kelseyhightower/confd \
        && cp $GOPATH/src/github.com/kelseyhightower/confd/bin/confd /usr/local/bin/confd; \
    else \
        # Download confd
