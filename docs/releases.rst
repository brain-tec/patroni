...
- Check ``NOTIFY_SOCKET`` environment variable before importing and using ``systemd`` package (Polina Bungina)

  This prevents ``FileNotFoundError: [Errno 2] No such file or directory`` exception when package is installed but socket does not exist and systemd integration is not intended to be used.
